...
Minion extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
minion channels login --channel zalouser
...
```bash
minion message send --channel zalouser --target <threadId> --message "Hello from Minion!"
```
...
```bash
minion channels login --channel zalouser              # Login via QR
minion channels login --channel zalouser --account work
minion channels status --probe
minion channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
minion directory self --channel zalouser
minion directory peers list --channel zalouser --query "name"
minion directory groups list --channel zalouser --query "work"
minion directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
minion message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
minion message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `minion channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
minion channels login --channel zalouser --account work
minion message send --channel zalouser --account work --target <id> --message "Hello"
