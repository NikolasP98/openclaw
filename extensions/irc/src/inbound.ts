...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as MinionConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as MinionConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as MinionConfig);
