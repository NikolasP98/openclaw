...
function resolveMinionRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.MINION_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set MINION_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveMinionRoot(), "dist", "extensionAPI.js");
