...
Designed to be called from workflow engines (for example, Squid via
`minion.invoke --each`) without adding new Minion code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Squid) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on Minion internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Minion extension (like `squid`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.minion/extensions` as a standalone plugin directory.
