...
// NOTE: This extension is intended to be bundled with Minion.
// When running from source (tests/dev), Minion internals live under src/.
...
export function createLlmTaskTool(api: MinionPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Squid workflows via minion.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-llm-task-"));
