...
metadata: { "minion": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Minion `sessions_spawn`
- **File I/O** == Minion `read`/`write`
- **Remote fetch** == Minion `web_fetch` (or `exec` with curl when POST is required)
