...
This document describes how the Minion ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`minion acp` exposes an ACP agent over stdio and forwards prompts to a running
Minion Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Minion Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `minion acp` over stdio.

Example config:

```bash
minion config set gateway.remote.url wss://gateway-host:18789
minion config set gateway.remote.token <token>
```

Example run:

```bash
minion acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
minion acp --session agent:main:main
minion acp --session agent:design:main
minion acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Minion ACP” to start a thread.

## Execution Model

- ACP client spawns `minion acp` and speaks ACP messages over stdio.
...
```bash
minion acp --session agent:main:main
minion acp --session-label "support inbox"
minion acp --reset-session
```
...
`minion acp` resolves the Gateway URL and auth from CLI flags or config:
