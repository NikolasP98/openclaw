...
INSTALL_URL="${MINION_INSTALL_URL:-https://minion.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MINION_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MINION_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="minion"
PACKAGE_NAME="${MINION_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MINION_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MINION_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MINION_INSTALL_LATEST_OUT:-}"
fi
