...
IMAGE_NAME="minion-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="minion-net-e2e-$$"
GW_NAME="minion-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "MINION_GATEWAY_TOKEN=$TOKEN" \
	  -e "MINION_SKIP_CHANNELS=1" \
	  -e "MINION_SKIP_GMAIL_WATCHER=1" \
	  -e "MINION_SKIP_CRON=1" \
	  -e "MINION_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
