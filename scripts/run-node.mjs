...
const shouldBuild = (deps) => {
  if (deps.env.MINION_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MINION_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[minion] ${message}\n`);
};

const runMinion = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["minion.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
