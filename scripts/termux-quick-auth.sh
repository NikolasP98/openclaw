...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/minion/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Minion Re-Auth" -c "After getting key, run: ssh $SERVER '~/minion/scripts/mobile-reauth.sh'" --id minion-auth
        ;;
    *)
