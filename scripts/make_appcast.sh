...
ZIP=${1:?"Usage: $0 Minion-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/minion/minion/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Minion-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/minion/minion/releases/download/v${VERSION}/"}
