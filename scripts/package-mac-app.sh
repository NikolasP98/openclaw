...
# Build and bundle Minion into a minimal .app we can open.
# Outputs to dist/Minion.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Minion.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Minion"
BUNDLE_ID="${BUNDLE_ID:-ai.minion.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/minion/minion/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Minion/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MinionBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MinionGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Minion"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Minion"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Minion"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Minion" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Minion/Resources/Minion.icns" "$APP_ROOT/Contents/Resources/Minion.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Minion/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MinionKit resources"
MINIONKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MinionKit_MinionKit.bundle"
if [ -d "$MINIONKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MinionKit_MinionKit.bundle"
  cp -R "$MINIONKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MinionKit_MinionKit.bundle"
else
  echo "WARN: MinionKit resource bundle not found at $MINIONKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Minion"
killall -q Minion 2>/dev/null || true
