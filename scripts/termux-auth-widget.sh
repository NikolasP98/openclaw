...
SERVER="${MINION_SERVER:-${MINIONBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Minion auth..."

STATUS=$(ssh "$SERVER" '$HOME/minion/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/minion/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|MINION_EXPIRING|MINIONBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/minion/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|MINION_EXPIRED|MINIONBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Minion won't work until re-auth"
                ;;
        esac
        ;;
