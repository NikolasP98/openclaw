#!/bin/bash
# Setup Minion Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'minion-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Minion message: Send warning via Minion itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/minion-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/minion-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/minion-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now minion-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status minion-auth-monitor.timer"
echo ""
echo "Quick commands:"
