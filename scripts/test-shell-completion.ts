 ...
 * `minion update`. Use it to verify the completion installation flow
...
\`minion update\`, \`minion doctor\`, and \`minion onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Minion Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`minion completion --install\` later to enable.`));
    return;
  }
