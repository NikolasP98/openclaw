...
#
# Suggested cron: */30 * * * * /home/admin/minion/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Minion notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., minion-alerts)
...
STATE_FILE="$HOME/.minion/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Minion if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use minion
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Minion to $NOTIFY_PHONE..."
            minion send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Minion is down. Run: ssh l36 '~/minion/scripts/mobile-reauth.sh'" "urgent"
    exit 1
