...
function ensureExperimentalWarningSuppressed(): boolean {
  ...
  if (isTruthyEnvValue(process.env.MINION_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.MINION_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.MINION_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[minion] ${parsed.error}`);
    process.exit(2);
  }
