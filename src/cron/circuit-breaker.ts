...
export class CronCircuitBreaker {
  ...
  recordFailure(jobName: string): string | undefined {
    ...
    if (job.state === "half-open") {
      ...
      log.warn(`Circuit re-opened for ${jobName} (probe failed, ${job.consecutiveFailures} total failures)`);
    ...
    if (job.consecutiveFailures >= this.config.maxFailures && job.state === "closed") {
      ...
      log.warn(`Circuit OPEN for ${jobName}: ${job.consecutiveFailures} consecutive failures. Suspended for ${cooldownMin}m.`);
