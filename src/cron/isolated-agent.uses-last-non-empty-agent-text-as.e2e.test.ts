...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "minion-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".minion", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<MinionConfig> = {},
): MinionConfig {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".minion", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
