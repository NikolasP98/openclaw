...
describe("discord tool result dispatch", () => {
      ...
      const cfg = {
        agents: {
          defaults: {
            ...
            workspace: "/tmp/minion",
          },
        },
        session: { store: "/tmp/minion-sessions.json" },
        channels: {
          discord: {
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Minion" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/minion" },
      session: { store: "/tmp/minion-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
