import type { BrowserConfig, BrowserProfileConfig, MinionConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_MINION_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "minion" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_MINION_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "minion" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MINION_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MINION_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Minion browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for minion/minion CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: MinionConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MINION_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "minion";
