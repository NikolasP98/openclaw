...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and squid-orange color", () => {
    ...
    expect(minion?.driver).toBe("minion");
    expect(minion?.cdpPort).toBe(18800);
    expect(minion?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from MINION_GATEWAY_PORT when unset", () => {
    const prev = process.env.MINION_GATEWAY_PORT;
    process.env.MINION_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const minion = resolveProfile(resolved, "minion");
      expect(minion?.cdpPort).toBe(19012);
      expect(minion?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.MINION_GATEWAY_PORT;
      } else {
        process.env.MINION_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const minion = resolveProfile(resolved, "minion");
      expect(minion?.cdpPort).toBe(19022);
      expect(minion?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.MINION_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "minion");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        minion: { cdpPort: 18792, color: "#FF4500" },
      },
    });
