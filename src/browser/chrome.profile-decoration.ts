...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".minion-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + squid-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateMinionProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MINION_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MINION_BROWSER_COLOR).toUpperCase();
