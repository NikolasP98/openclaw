  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          minion: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.MINION_GATEWAY_PORT;
    process.env.MINION_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.MINION_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.MINION_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.MINION_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.MINION_GATEWAY_TOKEN;
    } else {
      process.env.MINION_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.MINION_GATEWAY_PASSWORD;
    } else {
      process.env.MINION_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
