  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          minion: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    process.env.MINION_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MINION_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MINION_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPassword === undefined) {
      ...
      process.env.MINION_GATEWAY_PASSWORD = prevGatewayPassword;
    }
