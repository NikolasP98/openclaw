...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "minion-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    MINION_GATEWAY_TOKEN: "test-token",
    MINION_CONFIG_DIR: join(sandbox.rootDir, "config"),
    MINION_WORKSPACE_DIR: join(sandbox.rootDir, "minion"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        MINION_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        MINION_EXTRA_MOUNTS: undefined,
        MINION_HOME_VOLUME: "minion-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MINION_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("MINION_EXTRA_MOUNTS=");
    expect(envFile).toContain("MINION_HOME_VOLUME=minion-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("minion-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("minion-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg MINION_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
