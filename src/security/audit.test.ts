...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: MinionConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.MINION_GATEWAY_TOKEN;
    const prevPassword = process.env.MINION_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.MINION_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.MINION_GATEWAY_PASSWORD;
      } else {
        process.env.MINION_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "minion.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "minion.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: MinionConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.MINION_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-discord-"));
    process.env.MINION_STATE_DIR = tmp;
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "minion-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.MINION_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.MINION_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-discord-open-"));
    process.env.MINION_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MinionConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.MINION_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-slack-"));
    process.env.MINION_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MinionConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.MINION_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-slack-open-"));
    process.env.MINION_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MinionConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.MINION_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-telegram-"));
    process.env.MINION_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MinionConfig = {
  ...
  it("warns when Telegram allowFrom entries are non-numeric (legacy @username configs)", async () => {
    const prevStateDir = process.env.MINION_STATE_DIR;
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "minion-security-audit-telegram-invalid-allowfrom-"),
    );
    process.env.MINION_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: MinionConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.MINION_GATEWAY_TOKEN;
    process.env.MINION_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: MinionConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.minion",
      configPath: "/Users/test/Dropbox/.minion/minion.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: MinionConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "minion.json"),
      });
  ...
  it("flags enabled extensions when tool policy can expose plugin tools", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "minion-security-audit-plugins-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(path.join(stateDir, "extensions", "some-plugin"), {
      recursive: true,
      mode: 0o700,
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-audit-scanner-"));
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    it("prefers env password over remote password", async () => {
      process.env.MINION_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: MinionConfig = {
        gateway: {
          mode: "remote",
