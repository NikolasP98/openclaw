...
import { scheduleGatewaySigusr1Restart, triggerMinionRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Minion in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMinionRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Minion via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
