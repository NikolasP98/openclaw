...
function snapshotStateEnv(): StateEnvSnapshot {
  return { MINION_STATE_DIR: process.env.MINION_STATE_DIR };
}

function restoreStateEnv(snapshot: StateEnvSnapshot) {
  if (snapshot.MINION_STATE_DIR === undefined) {
    delete process.env.MINION_STATE_DIR;
  } else {
    process.env.MINION_STATE_DIR = snapshot.MINION_STATE_DIR;
  }
}
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "minion-typing-heartbeat-"));
  });
