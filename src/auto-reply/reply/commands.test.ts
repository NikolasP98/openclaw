...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-commands-"));
...
function buildParams(commandBody: string, cfg: MinionConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
