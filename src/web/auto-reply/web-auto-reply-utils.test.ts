...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\bminion\b/i] };

  it("ignores regex matches when other mentions are present", () => {
    const msg = makeMsg({
      body: "@Minion please help",
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { mentionRegexes: [/\bminion\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
      body: "@owner ping",
...
describe("resolveMentionTargets with @lid mapping", () => {
  it("resolves mentionedJids via lid reverse mapping in authDir", async () => {
    const authDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-lid-mapping-"));
    try {
      await fs.writeFile(
