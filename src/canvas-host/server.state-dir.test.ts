...
describe("canvas host state dir defaults", () => {
  ...
  it("uses MINION_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "minion-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Minion Canvas");
    } finally {
      await handler.close();
