...
export function listAgentEntries(cfg: MinionConfig): AgentEntry[] {
...
function resolveAgentName(cfg: MinionConfig, agentId: string) {
...
function resolveAgentModel(cfg: MinionConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MinionConfig): AgentSummary[] {
