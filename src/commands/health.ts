...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.MINION_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: MinionConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.MINION_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
