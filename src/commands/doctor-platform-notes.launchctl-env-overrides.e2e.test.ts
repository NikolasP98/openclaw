...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "MINION_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MINION_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MINION_GATEWAY_TOKEN");
    expect(message).not.toContain("MINION_GATEWAY_PASSWORD");
  });
