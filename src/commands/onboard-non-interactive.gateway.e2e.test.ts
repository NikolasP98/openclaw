...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "minion-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.MINION_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "minion");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.MINION_STATE_DIR = stateDir;
    process.env.MINION_CONFIG_PATH = path.join(stateDir, "minion.json");
