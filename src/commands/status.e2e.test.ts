...
vi.mock("../infra/minion-root.js", () => ({
  resolveMinionPackageRoot: vi.fn().mockResolvedValue("/tmp/minion"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/minion/pnpm-lock.yaml",
      markerPath: "/tmp/minion/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Minion status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("minion status --all") ||
          l.includes("minion --profile isolated status --all") ||
          l.includes("minion status --all") ||
          l.includes("minion --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.MINION_GATEWAY_TOKEN = "abcd1234";
    try {
