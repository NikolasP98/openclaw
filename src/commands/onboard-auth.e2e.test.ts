...
const requireAgentDir = () => {
  const agentDir = process.env.MINION_AGENT_DIR;
  if (!agentDir) {
    throw new Error("MINION_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.MINION_OAUTH_DIR;
  });

  it("writes auth-profiles.json under MINION_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-oauth-"));
    process.env.MINION_STATE_DIR = tempStateDir;
    process.env.MINION_AGENT_DIR = path.join(tempStateDir, "agent");
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MINION_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-minimax-"));
    process.env.MINION_STATE_DIR = tempStateDir;
    process.env.MINION_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.MINION_AGENT_DIR;
