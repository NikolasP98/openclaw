...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveMinionAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("minion models auth setup-token")}\` or \`${formatCliCommand("minion configure")}\`.`
          : `Run \`${formatCliCommand("minion configure")}\` or set an API key env var.`;
