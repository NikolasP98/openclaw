...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMinionAgentDir: vi.fn().mockReturnValue("/tmp/minion-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/minion-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/minion-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveMinionAgentDir: mocks.resolveMinionAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMinionAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/minion-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/minion-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/minion-agent-custom");
