...
import type { MinionConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: MinionConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("minion security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("minion configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("minion config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("minion doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("minion config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('minion config set session.dmScope "per-channel-peer"') +
