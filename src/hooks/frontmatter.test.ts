...
`;
    ...
    expect(parsed.minion.emoji).toBe("💾");
    expect(parsed.minion.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.minion.emoji).toBe("📝");
    expect(parsed.minion.events).toEqual(["command"]);
    expect(parsed.minion.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.minion.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"minion": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"minion": {"events": ["test"]}}');
  });
...
describe("resolveMinionMetadata", () => {
  it("extracts minion metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMinionMetadata(frontmatter);
  ...
  it("returns undefined when minion key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        minion: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Minion" },
            { id: "npm", kind: "npm", package: "@minion/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@minion/hook");
  });
...
`;
    ...
    const minion = resolveMinionMetadata(frontmatter);
    expect(minion).toBeDefined();
    expect(minion?.emoji).toBe("💾");
    expect(minion?.events).toEqual(["command:new"]);
    expect(minion?.requires?.config).toEqual(["workspace.dir"]);
    expect(minion?.install?.[0].kind).toBe("bundled");
  });
