...
const fixtureRoot = path.join(os.tmpdir(), `minion-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
beforeAll(async () => {
  ...
  zipHooks.file(
    ...
    JSON.stringify({
      name: "@minion/zip-hooks",
      version: "0.0.1",
      minion: { hooks: ["./hooks/zip-hook"] },
    }),
  );
  zipHooks.file(
    ...
    [
      ...
      'metadata: {"minion":{"events":["command:new"]}}',
      "---",
      "",
      "# Zip Hook",
    ].join("\n"),
  ...
  const makeTarWithPackage = async (params: {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        minion: { hooks: [`./hooks/${params.hookName}`] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  const makeNpmPackTgz = async (): Promise<Buffer> => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({
        name: "@minion/test-hooks",
        version: "0.0.1",
        minion: { hooks: ["./hooks/one-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@minion/test-hooks@0.0.1", "--ignore-scripts"]);
