...
homepage: https://docs.minion.ai/automation/hooks#command-logger
metadata:
  {
    "minion":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.minion/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.minion/logs/commands.log ~/.minion/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/minion`:
   ```
   /home/username/.minion/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
