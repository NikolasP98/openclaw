...
This directory contains hooks that ship with Minion. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.minion/minion.json`:
...
2. Restart gateway: `pkill -9 -f 'minion.*gateway' && pnpm minion gateway`
3. Enable the hook: `minion hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.minion.ai/automation/hooks
