...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "minion-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "minion-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "minion-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "minion-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.minion when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "minion-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers MINION_HOME over HOME", () => {
    vi.stubEnv("MINION_HOME", "/srv/minion-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/minion-home"));
...
describe("shortenHomePath", () => {
  it("uses $MINION_HOME prefix when MINION_HOME is set", () => {
    vi.stubEnv("MINION_HOME", "/srv/minion-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/minion-home")}/.minion/minion.json`)).toBe(
      "$MINION_HOME/.minion/minion.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $MINION_HOME replacement when MINION_HOME is set", () => {
    vi.stubEnv("MINION_HOME", "/srv/minion-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/minion-home")}/.minion/minion.json`),
    ).toBe("config: $MINION_HOME/.minion/minion.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/minion")).toBe(path.resolve(os.homedir(), "minion"));
  });
  ...
  it("prefers MINION_HOME for tilde expansion", () => {
    vi.stubEnv("MINION_HOME", "/srv/minion-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/minion")).toBe(path.resolve("/srv/minion-home", "minion"));
