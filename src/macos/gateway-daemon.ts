...
declare const __MINION_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __MINION_VERSION__ === "string" && __MINION_VERSION__) ||
  process.env.MINION_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `minion --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.MINION_GATEWAY_PORT ??
    process.env.MINIONBOT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.MINION_GATEWAY_BIND ??
    process.env.MINIONBOT_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
    bindRaw === "loopback" ||
