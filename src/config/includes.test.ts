...
const ETC_MINION_DIR = path.join(ROOT_DIR, "etc", "minion");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "minion.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcMinionPath(...parts: string[]) {
  return path.join(ETC_MINION_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcMinionPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "minion.json"))).toEqual({
      shared: true,
    });
  });
});
