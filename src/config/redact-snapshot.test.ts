...
import type { ConfigFileSnapshot } from "./types.minion.js";
import {
  REDACTED_SENTINEL,
...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.minion/config.json5",
    exists: true,
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = MinionSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "MinionConfig";
    const hints = mapSensitivePaths(MinionSchema, "", {});
