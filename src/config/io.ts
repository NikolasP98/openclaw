...
import type { MinionConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareMinionVersions } from "./version.js";
...
function coerceConfig(value: unknown): MinionConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as MinionConfig;
}
...
function stampConfigVersion(cfg: MinionConfig): MinionConfig {
...
function warnIfConfigFromFuture(cfg: MinionConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareMinionVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Minion (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MinionConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MinionConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as MinionConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: MinionConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MinionConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MINION_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.MINION_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.MINION_WATCH_SESSION === "string" &&
        deps.env.MINION_WATCH_SESSION.trim().length > 0
          ? deps.env.MINION_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.MINION_WATCH_COMMAND === "string" &&
        deps.env.MINION_WATCH_COMMAND.trim().length > 0
          ? deps.env.MINION_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `MINION_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MINION_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MINION_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
