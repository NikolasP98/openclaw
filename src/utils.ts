...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".minion");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.MINION_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$MINION_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via MINION_STATE_DIR.
