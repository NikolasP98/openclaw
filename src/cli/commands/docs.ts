...
const SEARCH_TOOL = "https://docs.minion.ai/mcp.SearchMinion";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.minion.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('minion docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.minion.ai/");
      runtime.log(`Search: ${formatCliCommand('minion docs "your query"')}`);
    }
    return;
  }
