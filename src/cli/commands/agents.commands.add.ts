...
async function writePerAgentConfig(
  ...
  try {
    ...
    if ((err as { code?: string }).code !== "EEXIST") {
      // Non-fatal: per-agent config creation failure doesn't block agent provisioning.
    }
  }
}
...
export async function agentsAddCommand(
  ...
  try {
    await prompter.intro("Add Minion agent");
    const name =
      nameInput ??
      (await prompter.text({
        message: "Agent name",
      ...
      if (wantsBindings) {
        ...
        await prompter.note(
          [
            ...
            "Docs: https://docs.minion.ai/concepts/multi-agent",
          ].join("\n"),
          "Routing",
        );
      }
