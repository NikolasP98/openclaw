...
import { resolveWorkspaceTemplateDir } from "../../agents/identity/workspace-templates.js";
import { resolveDefaultAgentWorkspaceDir } from "../../agents/identity/workspace.js";
import { handleReset } from "../../cli/commands/onboard-helpers.js";
...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = env.MINION_PROFILE?.trim().toLowerCase();
  if (profile === "dev") {
    return baseDir;
  }
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - Minion Dev Workspace\n\nDefault dev workspace for minion gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",
