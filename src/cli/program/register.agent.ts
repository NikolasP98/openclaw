...
${formatHelpExamples([
  ['minion agent --to +15555550123 --message "status update"', "Start a new session."],
  ['minion agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'minion agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'minion agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['minion agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'minion agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.minion.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.minion.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['minion agents set-identity --agent main --name "Minion" --emoji "🦑"', "Set name + emoji."],
  ["minion agents set-identity --agent main --avatar avatars/minion.png", "Set avatar path."],
  [
    "minion agents set-identity --workspace ~/.minion/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "minion agents set-identity --identity-file ~/.minion/workspace/IDENTITY.md --agent main",
