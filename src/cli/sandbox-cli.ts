...
const SANDBOX_EXAMPLES = {
  main: [
    ["minion sandbox list", "List all sandbox containers."],
    ["minion sandbox list --browser", "List only browser containers."],
    ["minion sandbox recreate --all", "Recreate all containers."],
    ["minion sandbox recreate --session main", "Recreate a specific session."],
    ["minion sandbox recreate --agent mybot", "Recreate agent containers."],
    ["minion sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["minion sandbox list", "List all sandbox containers."],
    ["minion sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["minion sandbox recreate --all", "Recreate all containers."],
    ["minion sandbox recreate --session main", "Recreate a specific session."],
    ["minion sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["minion sandbox recreate --browser --all", "Recreate only browser containers."],
    ["minion sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["minion sandbox explain", "Show effective sandbox config."],
    ["minion sandbox explain --session agent:main:main", "Explain a specific session."],
    ["minion sandbox explain --agent work", "Explain an agent sandbox."],
    ["minion sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.minion.ai/cli/sandbox")}\n`,
    )
    .action(() => {
