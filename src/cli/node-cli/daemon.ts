...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("minion node install"), formatCliCommand("minion node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("minion node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    MINION_LOG_PREFIX: baseEnv.MINION_LOG_PREFIX ?? "node",
