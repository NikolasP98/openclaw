...
describe("edge cases", () => {
  ...
  it("keeps .co domains as links", () => {
    const result = markdownToTelegramHtml("Visit t.co and minion.co");
    expect(result).toContain('<a href="http://t.co">');
    expect(result).toContain('<a href="http://minion.co">');
    expect(result).not.toContain("<code>t.co</code>");
    expect(result).not.toContain("<code>minion.co</code>");
  });
  ...
  it("handles nested path without domain-like segments", () => {
    const result = markdownToTelegramHtml("src/shared/helpers/format.go");
    expect(result).toContain("<code>src/shared/helpers/format.go</code>");
  });
