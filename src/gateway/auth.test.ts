...
describe("gateway auth", () => {
  it("resolves token/password from MINION gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          MINION_GATEWAY_TOKEN: "env-token",
          MINION_GATEWAY_PASSWORD: "env-password",
  ...
  it("does not resolve legacy MINIONBOT gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          MINIONBOT_GATEWAY_TOKEN: "legacy-token",
          MINIONBOT_GATEWAY_PASSWORD: "legacy-password",
        } as NodeJS.ProcessEnv,
      }),
    ).toMatchObject({
      mode: "none",
