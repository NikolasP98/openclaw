...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.MINION_CONFIG_PATH;
  const prevDisableCache = process.env.MINION_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "minion-plugin-http-auth-test-"));
  const configPath = path.join(dir, "minion.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.MINION_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.MINION_DISABLE_CONFIG_CACHE;
    } else {
      process.env.MINION_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
