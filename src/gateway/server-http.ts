...
import { handleSlackHttpRequest } from "../channels/impl/slack/http/index.js";
...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-Minion-Token header (query parameters are not allowed).",
      );
      return true;
    }
