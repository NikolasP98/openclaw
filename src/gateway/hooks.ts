...
export function resolveHooksConfig(cfg: MinionConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MinionConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-minion-token"] === "string" ? req.headers["x-minion-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
