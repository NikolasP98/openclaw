...
export async function startGatewayServer(
  ...
  // Declared separately so we can attach applyHotReload after createGatewayReloadHandlers (below).
  const requestContext: Parameters<typeof attachGatewayWsHandlers>[0]["context"] = {
    deps,
    cron,
    cronStorePath,
    execApprovalManager,
  ...
  const configReloader = minimalTestGateway
    ...
    : (() => {
        ...
        // Wire applyHotReload into the request context so config.patch/apply can use it
        requestContext.applyHotReload = applyHotReload;
