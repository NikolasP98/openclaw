...
describe("resolveMinionAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-agent-"));
    process.env.MINION_STATE_DIR = tempStateDir;
    delete process.env.MINION_AGENT_DIR;
    delete process.env.PI_CODING_AGENT_DIR;

    const resolved = resolveMinionAgentDir();

    expect(resolved).toBe(path.join(tempStateDir, "agents", "main", "agent"));
  });

  it("honors MINION_AGENT_DIR overrides", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-agent-"));
    const override = path.join(tempStateDir, "agent");
    process.env.MINION_AGENT_DIR = override;
    delete process.env.PI_CODING_AGENT_DIR;
