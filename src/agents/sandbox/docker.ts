...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "minion.sandbox=1");
  args.push("--label", `minion.sessionKey=${params.scopeKey}`);
  args.push("--label", `minion.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `minion.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "minion.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`minion sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`minion sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("minion sandbox recreate --all");
}
