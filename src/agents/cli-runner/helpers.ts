...
/**
 * Cleanup suspended Minion CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: MinionConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "minion-cli-images-"));
  const paths: string[] = [];
