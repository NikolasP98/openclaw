...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("minion-ws-", async (workspaceDir) => {
      await withTempDir("minion-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createMinionCodingTools({ workspaceDir });
    ...
    await withTempDir("minion-ws-", async (workspaceDir) => {
      await withTempDir("minion-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello minion");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("minion-ws-", async (workspaceDir) => {
      await withTempDir("minion-override-", async (overrideDir) => {
        const tools = createMinionCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("minion-sandbox-", async (sandboxDir) => {
      await withTempDir("minion-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createMinionCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
