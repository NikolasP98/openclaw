...
import { executeFanout, formatFanoutResults, type FanoutTask, type SpawnFn } from "./parallel-fanout.js";
...
describe("parallel-fanout", () => {
  ...
  it("handles failures in individual tasks", async () => {
    const failingSpawn: SpawnFn = async (task) => {
      if (task.label === "task-1") throw new Error("boom");
  ...
  it("handles all tasks failing", async () => {
    const allFail: SpawnFn = async () => { throw new Error("fail"); };
  ...
  describe("formatFanoutResults", () => {
    ...
    it("shows FAILED for failed tasks", async () => {
      const failSpawn: SpawnFn = async () => { throw new Error("oops"); };
