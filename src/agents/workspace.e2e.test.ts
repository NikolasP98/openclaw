...
import { makeTempWorkspace, writeWorkspaceFile } from "../test-support/workspace.js";
import {
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MINION_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MINION_HOME: "/srv/minion-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/minion-home"), ".minion", "workspaces"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".minion", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("minion-workspace-");
