...
import { resolvePreferredMinionTmpDir } from "../infra/tmp-minion-dir.js";
import { readLoggingConfig } from "./config.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredMinionTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "minion.log"); // legacy single-file path

const LOG_PREFIX = "minion";
...
function resolveSettings(): ResolvedSettings {
  let cfg: MinionConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => MinionConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.MINION_TEST_FILE_LOG !== "1" ? "silent" : "info";
