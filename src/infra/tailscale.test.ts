...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.MINION_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.MINION_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.MINION_TEST_TAILSCALE_BINARY;
    } else {
      process.env.MINION_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
