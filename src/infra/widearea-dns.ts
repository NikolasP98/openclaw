...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MINION_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*minion-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "minion";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "minion");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "minion-gw");
  ...
  records.push(`_minion-gw._tcp IN PTR ${instanceLabel}._minion-gw._tcp`);
  records.push(`${instanceLabel}._minion-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._minion-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; minion-content-hash: ${contentHash}\n${contentBody}`;
}
