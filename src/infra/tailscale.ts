...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.MINION_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Minion. You can keep running the web gateway without it: \`${formatCliCommand("minion gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
