...
function candidateBinDirs(opts: EnsureMinionPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `minion` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "minion");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MINION_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "minion"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `minion` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMinionCliOnPath(opts: EnsureMinionPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MINION_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.MINION_PATH_BOOTSTRAPPED = "1";
