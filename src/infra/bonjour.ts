...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MINION_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Minion";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Minion\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.MINION_MDNS_HOSTNAME?.trim() ||
    process.env.MINIONBOT_MDNS_HOSTNAME?.trim() ||
    "minion";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
