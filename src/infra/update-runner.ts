...
const DEFAULT_PACKAGE_NAME = "@nikolasp98/minion";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "minion-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "minion.mjs");
    ...
    const doctorStep = await runStep(
      step("minion doctor", doctorArgv, gitRoot, { MINION_UPDATE_IN_PROGRESS: "1" }),
    );
