...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/minion-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Minion" },
        },
      },
      me: { id: 999, username: "minion_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
