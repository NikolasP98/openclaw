...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `minion-e2e-${name}-`));
  const configDir = path.join(homeDir, ".minion");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MINION_SKIP_BROWSER_CONTROL_SERVER: "1",
