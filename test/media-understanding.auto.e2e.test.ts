...
import type { MinionConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("minion-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("minion-bin-sherpa-");
      const modelDir = await makeTempDir("minion-sherpa-model-");
      ...
      const cfg: MinionConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("minion-bin-whispercpp-");
      const modelDir = await makeTempDir("minion-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("minion-bin-gemini-");
      ...
      const cfg: MinionConfig = { tools: { media: { image: {} } } };
