...
- **Design for opt-in** — Gateway features that are off by default should include an env var toggle (`MINION_*`) from the start, using `isTruthyEnvValue` + `logAcceptedEnvOption` from `src/infra/env.ts`
