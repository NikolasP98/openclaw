---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Minion deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Minion cron scheduling for periodic checks, or version status checks on a machine running Minion (laptop, workstation, Pi, VPS).
---

# Minion Host Hardening

## Overview

Assess and harden the host running Minion, then align it to a user-defined risk tolerance without breaking access. Use Minion security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Minion changes the host firewall, SSH, or OS updates; it does not.
...
5. Minion gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Minion can access sensitive data.
...
### 2) Run Minion security audits (read-only)

As part of the default read-only checks, run `minion security audit --deep`. Only offer alternatives if the user requests them:

1. `minion security audit` (faster, non-probing)
2. `minion security audit --json` (structured output)

Offer to apply Minion safe defaults (numbered):

1. `minion security audit --fix`

Be explicit that `--fix` only tightens Minion defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Minion version/update status (read-only)

As part of the default read-only checks, run `minion update status`.
...
- Credential hygiene notes (location of Minion creds, prefer disk encryption)
...
After Minion install or first hardening pass, run at least one baseline audit and version check:

- `minion security audit`
- `minion security audit --deep`
- `minion update status`

Ongoing monitoring is recommended. Use the Minion cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `minion cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `minion update status`
...
Before creating, `minion cron list` and match on exact `name`. If found, `minion cron edit <id> ...`.
If not found, `minion cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `minion update status` (preferred for source checkouts and channels)
2. `npm view minion version` (published npm version)

## Minion command accuracy

Use only supported commands and flags:

- `minion security audit [--deep] [--fix] [--json]`
- `minion status` / `minion status --deep`
- `minion health --json`
- `minion update status`
- `minion cron add|list|runs|run`

Do not invent CLI flags or imply Minion enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Minion compaction:
