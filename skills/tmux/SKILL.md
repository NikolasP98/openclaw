...
metadata:
  { "minion": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${MINION_TMUX_SOCKET_DIR:-${MINIONBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/minion-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/minion.sock"
SESSION=minion-python
...
- Use `MINION_TMUX_SOCKET_DIR` (legacy `MINIONBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$MINION_TMUX_SOCKET_DIR/minion.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `MINION_TMUX_SOCKET_DIR`).
