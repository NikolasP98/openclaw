...
Deploy Minion on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/minionbot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/minion` — Control UI

## What you get

- Hosted Minion Gateway + Control UI
...
- `MINION_STATE_DIR=/data/.minion` (recommended)
- `MINION_WORKSPACE_DIR=/data/workspace` (recommended)
- `MINION_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Minion state + workspace so you can migrate to another host without losing config or memory.
