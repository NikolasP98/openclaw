...
Deploy Minion on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/minion/minion)
...
repository configures everything needed to run Minion:
...
The web dashboard is available at `https://<your-service>.onrender.com/minion`.
...
If you use the original Minion repository, Render will not auto-deploy your Minion. To update it, run a manual Blueprint sync from the dashboard.
...
For Minion, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Minion host.
