---
summary: "Agent tool surface for Minion (browser, canvas, nodes, message, cron) replacing legacy `minion-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `minion-*` skills
title: "Tools"
---

# Tools (Minion)

Minion exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `minion-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `minion.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Minion logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:minion`: all built-in Minion tools (excludes provider plugins)
...
- [Squid](/tools/squid): typed workflow runtime with resumable approvals (requires the Squid CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`minion node run`).
...
- Requires a Brave API key (recommended: `minion configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Minion-managed browser.
...
- Quick smoke: `minion nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `minion gateway` restart in-place)
