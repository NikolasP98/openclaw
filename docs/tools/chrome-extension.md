---
summary: "Chrome extension: let Minion drive your existing Chrome tab"
read_when:
...
The Minion Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate minion-managed Chrome profile.
...
Minion then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Minion release (npm package) as static files. There is no separate “build” step.

After upgrading Minion:

- Re-run `minion browser extension install` to refresh the installed files under your Minion state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Minion ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Minion to control.
...
- `ON`: attached; Minion can drive that tab.
...
Debugging: `minion sandbox explain`
...
`minion browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `minion browser extension install` first to copy the extension to a stable location under your Minion state directory.
...
- **This is not isolated** like the dedicated minion-managed profile.
