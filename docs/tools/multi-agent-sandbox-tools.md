...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `minion sandbox explain`.
...
Legacy `agent.*` configs are migrated by `minion doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=minion-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${MINION_STATE_DIR:-$HOME/.minion}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
