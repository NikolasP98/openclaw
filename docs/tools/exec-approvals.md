...
- **gateway host** → `minion` process on the gateway machine
...
`~/.minion/exec-approvals.json` directly.

CLI: `minion approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
