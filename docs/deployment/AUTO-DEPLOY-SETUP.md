...
This guide walks you through setting up automatic deployment for Minion to production servers.
...
The `MINION_GATEWAY_TOKEN` has already been generated by the setup script.
...
| `DEPLOYMENT_PATH` | `/home/deploy/minion-prd`                     | Deployment directory                  |
...
**Note**: The same SSH key (`~/.ssh/minion/minion_deploy_key`) is used for all servers.
...
```json
{
  "servers": [
    ...
    },
    {
      ...
      "user": "deploy",
      "port": 22,
      "deployment_path": "/home/deploy/minion-prd-widgets",
...
**Server 1 (ACME)**: `/home/deploy/minion-prd-acme/.env`
...
**Server 2 (Widgets)**: `/home/deploy/minion-prd-widgets/.env`

```bash
MINION_GATEWAY_CONTAINER_NAME=widgets_minion_gw
MINION_CLI_CONTAINER_NAME=widgets_minion_cli
MINION_CONFIG_DIR=/home/deploy/.minion-prd-widgets
MINION_WORKSPACE_DIR=/home/deploy/.minion-prd-widgets/workspace
# ... tenant-specific credentials
```
...
```bash
scp scripts/deployment/backup-minion.sh deploy@<server-ip>:~/
ssh deploy@<server-ip>
...
crontab -e
# Add this line:
0 2 * * * /home/deploy/backup-minion.sh
```
...
ssh deploy@<server-ip>
cd ~
tar -czf minion-backup-$(date +%Y%m%d).tar.gz .minion-prd minion-prd
```
...
docker compose down
cd ~
tar -xzf minion-backup-<date>.tar.gz
cd ~/minion-prd
docker compose up -d
```
