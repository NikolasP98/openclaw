# Minion Production Deployment - Implementation Checklist
...
- [ ] Verify private key created: `~/.ssh/minion/minion_deploy_key`
- [ ] Verify public key created: `~/.ssh/minion/minion_deploy_key.pub`
...
- [ ] Copy public key to server: `scp ~/.ssh/minion/minion_deploy_key.pub root@SERVER_IP:/tmp/`
- [ ] SSH to server as root: `ssh root@SERVER_IP`
- [ ] Run setup script: `cd /tmp && chmod +x setup-server.sh && ./setup-server.sh default minion_deploy_key.pub`
- [ ] Verify deploy user created: `id deploy`
- [ ] Verify deploy user in docker group: `groups deploy | grep docker`
- [ ] Verify deployment directory exists: `ls -la /home/deploy/minion-prd`
- [ ] Verify SSH key added: `su - deploy -c "cat ~/.ssh/authorized_keys"`

### Step 3: Configure Production Environment

- [ ] SSH to server as deploy user: `ssh -i ~/.ssh/minion/minion_deploy_key deploy@SERVER_IP`
- [ ] Edit .env file: `nano ~/minion-prd/.env`
...
- [ ] Verify `MINION_GATEWAY_TOKEN` is set (auto-generated by setup script)
- [ ] Save and exit
- [ ] Verify .env file permissions: `ls -la ~/minion-prd/.env` (should be 644)

### Step 4: Copy docker-compose.yml

- [ ] From local machine, copy file: `scp docker-compose.yml deploy@SERVER_IP:~/minion-prd/`
- [ ] Verify file copied: `ssh deploy@SERVER_IP "ls -la ~/minion-prd/docker-compose.yml"`

### Step 5: Test Manual Deployment

- [ ] SSH to server: `ssh -i ~/.ssh/minion/minion_deploy_key deploy@SERVER_IP`
- [ ] Navigate to deployment directory: `cd ~/minion-prd`
- [ ] Pull images: `docker compose pull`
- [ ] Verify images pulled: `docker images | grep minion`
...
- [ ] Check logs: `docker compose logs minion-gateway` (no critical errors)
...
- [ ] Add `SSH_PRIVATE_KEY`:
  ...
  - Value: Contents of `~/.ssh/minion/minion_deploy_key` (entire file including BEGIN/END lines)
  - Copy with: `cat ~/.ssh/minion/minion_deploy_key`
- [ ] Add `SSH_HOST`:
  - Name: `SSH_HOST`
...
- [ ] Check image version: `docker images | grep minion` (should show latest timestamp)
- [ ] Test health endpoint: `curl http://localhost:18789/health` (should return 200 OK)
- [ ] Check logs for new deployment: `docker compose logs --tail=50 minion-gateway`
...
- [ ] Copy backup script to server: `scp scripts/deployment/backup-minion.sh deploy@SERVER_IP:~/`
- [ ] Make executable: `ssh deploy@SERVER_IP 'chmod +x ~/backup-minion.sh'`
- [ ] Test backup: `ssh deploy@SERVER_IP '~/backup-minion.sh'`
- [ ] Verify backup created: `ssh deploy@SERVER_IP 'ls -la /var/backups/minion/'`
...
- [ ] Test SSH key access still works: `ssh -i ~/.ssh/minion/minion_deploy_key deploy@SERVER_IP`
...
## 📈 Phase 2: Multi-Server Deployment (Optional)
...
- [ ] SSH key permissions: `ls -la ~/.ssh/minion/`
...
- [x] ✅ Push to PRD branch triggers automatic deployment
- [x] ✅ Deployment completes without errors
- [x] ✅ Health checks pass
- [x] ✅ Containers are running on server
- [x] ✅ Gateway is accessible
- [x] ✅ Logs show no critical errors
- [x] ✅ You can trigger deployments without manual SSH
- [x] ✅ Automatic rollback works (if tested)
...
Once all items are checked, you have successfully implemented automatic production deployment for Minion!
