...
```bash
minion tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `minion tui --deliver`
...
- Check the Gateway logs: `minion logs --follow`.
- Confirm the agent can run: `minion status` and `minion models status`.
...
- No agents in picker: check `minion agents list` and your routing config.
