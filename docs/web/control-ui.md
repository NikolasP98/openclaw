...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/minion`)
...
If the page fails to load, start the Gateway first: `minion gateway`.
...
you revoke it with `minion devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.minion/minion.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Minion
...
Minion **blocks** Control UI connections without device identity.
