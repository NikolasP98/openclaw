...
read_when:
  - Running Minion behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Minion
...
- You run Minion behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Minion checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Minion extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://minion.example.com
    to: http://minion-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`minion security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Minion config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `minion security audit` and review findings
