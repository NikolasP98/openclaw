...
- During heartbeat runs, Minion treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `minion sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Minion skips the heartbeat run entirely (no model call).
...
```bash
minion system event --text "Check for urgent follow-ups" --mode now
```
