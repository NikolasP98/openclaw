...
description: "Complete field-by-field reference for ~/.minion/minion.json"
---

# Configuration Reference

Every field available in `~/.minion/minion.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Minion uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `minion doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["minion-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `minion plugins install @minion/mattermost`.
...
Minion spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@minion"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `minion.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Minion auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Minion assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.minion/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.minion/agents/main/agent",
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:minion`     | All built-in tools (excludes provider plugins)                                           |
...
Minion uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.minion/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `MINION_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `minion onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `minion onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `minion onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `minion onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `minion onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `minion onboard --auth-choice minimax-api`.
...
- Loaded from `~/.minion/extensions`, `<workspace>/.minion/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or MINION_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `MINION_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.minion-dev` + port `19001`), `--profile <name>` (uses `~/.minion-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.minion/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-minion-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `MINION_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or MINION_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__minion__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__minion__/a2ui/`
...
- Also serves A2UI at `/__minion__/a2ui/`.
...
- Hostname defaults to `minion`. Override with `MINION_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.minion/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `minion dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.minion/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${MINION_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.minion/credentials/oauth.json`.
...
- Default log file: `/tmp/minion/minion-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `minion doctor --fix` can strip unknown keys).
