...
read_when:
  - Designing workspace memory (~/.minion/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Minion integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.minion/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.minion/workspace` as canonical human-readable memory.
...
```
~/.minion/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Minion**, but keep a separable core library.

### Why integrate into Minion?

- Minion already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `minion memory recall "…" --k 25 --since 30d`
  - `minion memory reflect --since 7d`
...
Pragmatic take for `~/.minion/workspace`:
