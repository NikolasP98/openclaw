...
read_when:
  ...
  - Wiring external systems into Minion
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Minion config directory (typically `~/.minion/hooks/transforms`).
...
- `minion webhooks gmail setup` writes `hooks.gmail` config for `minion webhooks gmail run`.
