...
minion cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
minion cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
minion cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```

## Squid: Deterministic workflows with approvals

Squid is the workflow runtime for **multi-step tool pipelines** that need deterministic execution and explicit approvals.
...
- **Squid** defines _what steps_ happen once the run starts.

For scheduled workflows, use cron or heartbeat to trigger an agent turn that calls Squid.
For ad-hoc workflows, call Squid directly.

### Operational notes (from the code)

- Squid runs as a **local subprocess** (`squid` CLI) in tool mode and returns a **JSON envelope**.
- If the tool returns `needs_approval`, you resume with a `resumeToken` and `approve` flag.
- The tool is an **optional plugin**; enable it additively via `tools.alsoAllow: ["squid"]` (recommended).
- If you pass `squidPath`, it must be an **absolute path**.

See [Squid](/tools/squid) for full usage and examples.
