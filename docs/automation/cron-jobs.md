...
- Jobs persist under `~/.minion/cron/` so restarts don’t lose schedules.
...
minion cron list
minion cron run <job-id>
minion cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.minion/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `minion cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Minion defaults to `announce`.
...
- Job store: `~/.minion/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.minion/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
minion cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
minion cron edit <jobId> --agent ops
minion cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
minion cron run <jobId>
minion cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
minion cron edit <jobId> \
...
```bash
minion system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`minion system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `MINION_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Minion applies exponential retry backoff for recurring jobs after consecutive errors:
