...
- `/usage cost` in chats: local cost summary aggregated from Minion session logs.
- CLI: `minion status --usage` prints a full per-provider breakdown.
- CLI: `minion channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
