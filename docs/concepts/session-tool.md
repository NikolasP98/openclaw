...
- When given a `sessionId`, Minion resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, Minion runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, Minion runs the **agent‑to‑agent announce step** (target agent only):
...
- After completion, Minion runs a sub-agent **announce step** and posts the result to the requester chat channel.
