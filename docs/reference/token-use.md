---
summary: "How Minion builds prompt context and reports token usage + costs"
read_when:
...
Minion tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Minion assembles its own system prompt on every run. It includes:
...
- `/usage cost` → shows a local cost summary from Minion session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `minion status --usage` and `minion channels list` show
...
`cacheWrite`. If pricing is missing, Minion shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Minion can
