...
Minion integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Minion owns lifecycle when `channels.signal.autoStart=true`.
...
- Minion spawns `imsg rpc` as a child process (legacy iMessage integration).
