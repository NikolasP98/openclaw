...
Minion features that can generate provider usage or paid API calls.
...
- `minion status --usage` and `minion channels list` show provider **usage windows**
...
Minion can pick up credentials from:
...
`minion models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
