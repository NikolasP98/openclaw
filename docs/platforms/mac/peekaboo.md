...
Minion can host **PeekabooBridge** as a local, permission‑aware UI automation
...
When enabled, Minion starts a local UNIX socket server. If disabled, the host
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID + Minion app TeamID).
