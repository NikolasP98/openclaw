...
- Same path: run the app from a fixed location (for Minion, `dist/OpenClaw.app`).
...
Workaround: move files into the Minion workspace (`~/.minion/workspace`) if you want to avoid per-folder grants.
