---
summary: "Minion on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Minion on a Raspberry Pi
  - Running Minion on ARM devices
...
Run a persistent, always-on Minion Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Minion features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled minion
...
```bash
# Check logs
journalctl -u minion --no-pager -n 100

# Common fix: rebuild
cd ~/minion  # if using hackable install
npm run build
sudo systemctl restart minion
```
