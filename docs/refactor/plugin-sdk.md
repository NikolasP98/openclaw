...
- Publish as `minion/plugin-sdk` (or export from core under `minion/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `MinionPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: MinionConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: MinionConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: MinionConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: MinionConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: MinionConfig): string;
  };
};
```
...
- Introduce `minion/plugin-sdk`.
- Add `api.runtime` to `MinionPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `minionRuntime: ">=2026.2.0"`).
