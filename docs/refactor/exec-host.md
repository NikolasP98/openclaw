...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.minion/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.minion/node.json` (node id + pairing token).
...
Path: `~/.minion/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.minion/exec-approvals.sock",
...
- Unix socket at `~/.minion/exec-approvals.sock` (0600).
