...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Minion
...
Running Minion via Node with `tsx` fails at startup with:

```
[minion] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `minion status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch minion.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node minion.mjs status` works on Node 25.
