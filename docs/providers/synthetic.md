---
summary: "Use Synthetic's Anthropic-compatible API in Minion"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Minion registers it as the
...
Note: Minion's Anthropic client appends `/v1` to the base URL, so use
