---
summary: "Use MiniMax M2.1 in Minion"
read_when:
  - You want MiniMax models in Minion
...
```bash
minion plugins enable minimax-portal-auth  # skip if already loaded.
minion gateway restart  # restart if gateway is already running
minion onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/minion/minion/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `minion.json`:
...
- Use `minion models list` and `minion models set minimax/MiniMax-M2.1` to switch.
...
- Running `minion configure` and selecting **MiniMax M2.1**, or
