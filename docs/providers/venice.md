---
summary: "Use Venice AI privacy-focused models in Minion"
read_when:
  - You want privacy-focused inference in Minion
...
```bash
minion onboard --auth-choice venice-api-key
```
...
```bash
minion chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Minion shows all available Venice models. Pick based on your needs:
...
```bash
minion models set venice/claude-opus-45
minion models set venice/llama-3.3-70b
```

List all available models:

```bash
minion models list | grep venice
```

## Configure via `minion configure`

1. Run `minion configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Minion automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
minion chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
minion chat --model venice/claude-opus-45

# Use uncensored model
minion chat --model venice/venice-uncensored

# Use vision model with image
minion chat --model venice/qwen3-vl-235b-a22b

# Use coding model
minion chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `minion models list` to see currently available models. Some models may be temporarily offline.
