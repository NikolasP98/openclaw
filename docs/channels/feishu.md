...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Minion to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
minion plugins install @minion/feishu
```

Local checkout (when running from a git repo):

```bash
minion plugins install ./extensions/feishu
```
...
If you just installed Minion, run the wizard:
...
2. Configuring app credentials in Minion
...
1. You already ran `minion channels add` for Feishu
2. The gateway is running (`minion gateway status`)
...
```bash
minion pairing approve feishu <CODE>
```
...
2. Run `minion logs --follow` and look for `chat_id`
...
2. Run `minion logs --follow` and look for `open_id`
...
| Command                  | Description                   |
| ------------------------ | ----------------------------- |
| `minion gateway status`  | Show gateway status           |
| `minion gateway install` | Install/start gateway service |
| `minion gateway stop`    | Stop gateway service          |
| `minion gateway restart` | Restart gateway service       |
| `minion logs --follow`   | Tail gateway logs             |
...
4. Check logs: `minion logs --follow`
...
5. Ensure the gateway is running: `minion gateway status`
6. Check logs: `minion logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.minion/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.minion/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
