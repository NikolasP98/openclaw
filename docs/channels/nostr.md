...
read_when:
  - You want Minion to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Minion to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`minion onboard`) and `minion channels add` list optional channel plugins.
...
```bash
minion plugins install @minion/nostr
```

Use a local checkout (dev workflows):

```bash
minion plugins install --link <path-to-minion>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "minion@example.com",
        "lud16": "minion@example.com"
      }
    }
  }
}
