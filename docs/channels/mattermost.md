---
summary: "Mattermost bot setup and Minion config"
read_when:
...
```bash
minion plugins install @minion/mattermost
```

Local checkout (when running from a git repo):

```bash
minion plugins install ./extensions/mattermost
```
...
4. Configure Minion and start the gateway.
...
- Approve via:
  - `minion pairing list mattermost`
  - `minion pairing approve mattermost <CODE>`
...
Use these target formats with `minion message send` or cron/webhooks:
