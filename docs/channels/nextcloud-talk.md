...
```bash
minion plugins install @minion/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
minion plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Minion" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `minion pairing list nextcloud-talk`
  - `minion pairing approve nextcloud-talk <CODE>`
