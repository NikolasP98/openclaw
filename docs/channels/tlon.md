...
Tlon is a decentralized messenger built on Urbit. Minion connects to your Urbit ship and can
...
```bash
minion plugins install @minion/tlon
```

Local checkout (when running from a git repo):

```bash
minion plugins install ./extensions/tlon
```
...
By default, Minion blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `minion message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Minion replies in-thread.
