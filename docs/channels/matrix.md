...
Matrix is an open, decentralized messaging protocol. Minion connects as a Matrix **user**
...
```bash
minion plugins install @minion/matrix
```

Local checkout (when running from a git repo):

```bash
minion plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `minion plugins install @minion/matrix`
   - From a local checkout: `minion plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Minion calls the same
     login endpoint, stores the access token in `~/.minion/credentials/matrix/credentials.json`,
...
- On first connection, Minion requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Minion logs a warning.
...
`~/.minion/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `minion pairing list matrix`
  - `minion pairing approve matrix <CODE>`
...
- On startup, Minion resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
