...
read_when:
  - Setting up Twitch chat integration for Minion
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Minion connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
minion plugins install @minion/twitch
```

Local checkout (when running from a git repo):

```bash
minion plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `MINION_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "minion", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use MINION_TWITCH_ACCESS_TOKEN env var)
...
```bash
MINION_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
