---
read_when:
  - 制作或验证 Minion macOS 发布版本
  - 更新 Sparkle appcast 或订阅源资源
summary: Minion macOS 发布清单（Sparkle 订阅源、打包、签名）
title: macOS 发布
x-i18n:
...
- 用于 `xcrun notarytool` 的公证凭据（钥匙串配置文件或 API 密钥），以实现通过 Gatekeeper 安全分发的 DMG/zip。
  - 我们使用名为 `minion-notary` 的钥匙串配置文件，由 shell 配置文件中的 App Store Connect API 密钥环境变量创建：
