...
你可以在 `~/.minion/minion.json` 中覆盖此设置：
...
所有日志配置都在 `~/.minion/minion.json` 的 `logging` 下。
...
- Minion 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `minion plugins enable diagnostics-otel` 启用插件。
...
- `minion.tokens`（计数器，属性：`minion.token`、`minion.channel`、`minion.provider`、`minion.model`）
- `minion.cost.usd`（计数器，属性：`minion.channel`、`minion.provider`、`minion.model`）
- `minion.run.duration_ms`（直方图，属性：`minion.channel`、`minion.provider`、`minion.model`）
- `minion.context.tokens`（直方图，属性：`minion.context`、`minion.channel`、`minion.provider`、`minion.model`）

消息流：

- `minion.webhook.received`（计数器，属性：`minion.channel`、`minion.webhook`）
- `minion.webhook.error`（计数器，属性：`minion.channel`、`minion.webhook`）
- `minion.webhook.duration_ms`（直方图，属性：`minion.channel`、`minion.webhook`）
- `minion.message.queued`（计数器，属性：`minion.channel`、`minion.source`）
- `minion.message.processed`（计数器，属性：`minion.channel`、`minion.outcome`）
- `minion.message.duration_ms`（直方图，属性：`minion.channel`、`minion.outcome`）

队列 + 会话：

- `minion.queue.lane.enqueue`（计数器，属性：`minion.lane`）
- `minion.queue.lane.dequeue`（计数器，属性：`minion.lane`）
- `minion.queue.depth`（直方图，属性：`minion.lane` 或 `minion.channel=heartbeat`）
- `minion.queue.wait_ms`（直方图，属性：`minion.lane`）
- `minion.session.state`（计数器，属性：`minion.state`、`minion.reason`）
- `minion.session.stuck`（计数器，属性：`minion.state`）
- `minion.session.stuck_age_ms`（直方图，属性：`minion.state`）
- `minion.run.attempt`（计数器，属性：`minion.attempt`）
...
- `minion.webhook.error`
  - `minion.channel`、`minion.webhook`、`minion.chatId`、`minion.error`
- `minion.message.processed`
  - `minion.channel`、`minion.outcome`、`minion.chatId`、`minion.messageId`、`minion.sessionKey`、`minion.sessionId`、`minion.reason`
- `minion.session.stuck`
  - `minion.state`、`minion.ageMs`、`minion.queueDepth`、`minion.sessionKey`、`minion.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `minion doctor`。
