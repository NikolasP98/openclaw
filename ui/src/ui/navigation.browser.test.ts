...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/minion/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/minion");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/minion/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MINION_CONTROL_UI_BASE_PATH__ = "/minion";
    const app = mountApp("/minion/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/minion");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/minion/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("minion.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
