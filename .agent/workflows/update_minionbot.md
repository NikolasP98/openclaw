---
description: Update Minionbot from upstream when branch has diverged (ahead/behind)
---

# Minionbot Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Minionbot" || true

# Move old version
mv /Applications/Minionbot.app /tmp/Minionbot-backup.app

# Install new build
cp -R dist/Minionbot.app /Applications/

# Launch
open /Applications/Minionbot.app
```
...
pnpm minionbot health

# Verify no zombie processes
ps aux | grep -E "(minionbot|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm minionbot agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update minionbot.json with fallback chains
...
pnpm minionbot agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
